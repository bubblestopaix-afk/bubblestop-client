...
async function appelAgent(body: Record<string, unknown>): Promise<any> {
...
export default function Parrainage() {
  ...
  const partager = async () => {
    if (!etat.code) return;
    const texte = `Rejoins-moi chez Bubble Stop 🧋 Télécharge l'appli (cherche « bubblestop ») et entre mon code parrain ${etat.code} : tu gagnes ${etat.recompenses.filleul} tampons de bienvenue !`;
    try {
    ...
    <View style={styles.carte}>
      ...
      {etat.code ? (
        <>
          ...
          <BoutonPrimaire titre="Partager mon code" onPress={partager} />
